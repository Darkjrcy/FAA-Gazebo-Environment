...
class FollowATrajecotory(Node):
    def __init__(self):
    # Start the node:
        super().__init__('move_the_airplanes_in_MIT_trajectory')
        ...
        Trajectory_path = "/home/adcl/AirplanePathFollower/src/airplane_recognition/MIT_Trajectories/Trajectory_points.json" # Important if you want to do your own Trajecotory change the direction to yours and follow the smae format
        # Open the file:
        try:
        ...
        # Deifne the spawner characterisitcs:
        self.time = Trajectory_data["time"]
        self.ranges = []
        # Define the own minimum range position and orientation of the encounters:
        self.x_own = Trajectory_data["x_own"]
        self.y_own = Trajectory_data["y_own"]
        self.z_own = Trajectory_data["z_own"]
        self.vx_own = Trajectory_data["vx_own"]
        self.vy_own = Trajectory_data["vy_own"]
        self.vz_own = Trajectory_data["vz_own"]
        self.roll_own = Trajectory_data["roll_own"]
        self.pitch_own = Trajectory_data["pitch_own"]
        self.yaw_own = Trajectory_data["yaw_own"]
        # The same for the intruder:
        self.x_in = Trajectory_data["x_in"]
        self.y_in = Trajectory_data["y_in"]
        self.z_in = Trajectory_data["z_in"]
        self.vx_in = Trajectory_data["vx_in"]
        self.vy_in = Trajectory_data["vy_in"]
        self.vz_in = Trajectory_data["vz_in"]
        self.roll_in = Trajectory_data["roll_in"]
        self.pitch_in = Trajectory_data["pitch_in"]
        self.yaw_in = Trajectory_data["yaw_in"]
        ...
        # Create a subscription to the YOLO Algorith: 
    ...
    def wait_for_message(self, timeout=10):
        ...
        while time.time() - start_time < timeout:
            rclpy.spin_once(self, timeout_sec=0.5)  # Process callbacks
        self.get_logger().info("Finished waiting")
    
    # Code to follow the trajectory:
    def follow_the_trajecotory(self):
        # DO a forloop for all the required points:
        for i in range(len(self.time)):
            self.get_logger().info(f"Time: {i}")
            # Start teh recognition:
    ...
    def save_information(self):
        # Generate the folder with the airplane inormation:
        airplane_Data = os.path.join(self.output_folder, "state_data")
        os.makedirs(airplane_Data)
        #Own ariplane:
        own_airplane_file_name = 'own_airplane_states.csv'
        own_airplane_file = os.path.join(airplane_Data, own_airplane_file_name)
        # Save the information:
        with open(own_airplane_file, 'w') as file:
            file.write("Time_s,x_ft,y_ft,z_ft,vx_ftps,vy_ftps,vz_ftps,roll_rad,pitch_rad,yaw_rad\n")
            for i in range(len(self.x_own)):
                file.write(f"{self.time[i]},{self.x_own[i]},{self.y_own[i]},{self.z_in[i]},{self.vx_own[i]},{self.vy_own[i]},{self.vz_own[i]},{self.roll_own[i]},{self.pitch_own[i]},{self.yaw_own[i]}\n")

        #Intruder ariplane:
        in_airplane_file_name = 'intruder_airplane_states.csv'
        in_airplane_file = os.path.join(airplane_Data, in_airplane_file_name)
        # Save the information:
        with open(in_airplane_file, 'w') as file:
            file.write("Time_s,x_ft,y_ft,z_ft,vx_ftps,vy_ftps,vz_ftps,roll_rad,pitch_rad,yaw_rad\n")
            for i in range(len(self.x_in)):
                file.write(f"{self.time[i]},{self.x_in[i]},{self.y_in[i]},{self.z_in[i]},{self.vx_in[i]},{self.vy_in[i]},{self.vz_in[i]},{self.roll_in[i]},{self.pitch_in[i]},{self.yaw_in[i]}\n")
        ...
        with open(file_path_detection_process, 'w') as file:
            ...
            for i in range(len(self.time)):
                file.write(f"{self.time[i]},{self.ranges[i]},{self.front_camera_scores[i]},{self.rigth_front_camera_scores[i]},{self.rigth_back_camera_scores[i]},{self.left_back_camera_scores[i]},{self.left_front_camera_scores[i]},{self.preprocess_times[i]},{self.inference_times[i]},{self.postprocess_times[i]}\n") 
...
def main(args=None):
    ...
    follow_MIT = FollowATrajecotory()
    try:
        follow_MIT.follow_the_trajecotory()
        rclpy.spin(follow_MIT)  # Keep spinning to handle messages
    except Exception as e:
        follow_MIT.get_logger().error(f'Error during recognition: {e}')
    finally:
        # Save the data before shutting down
        follow_MIT.save_information()
        # Destroy the node explicitly
        follow_MIT.destroy_node()
